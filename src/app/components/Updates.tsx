...
import { Heart, MessageCircle, Mic, TrendingUp, Send, Sparkles } from 'lucide-react';
...
export function Updates() {
  ...
  const handleVoiceInput = () => {
    ...
    if (!isRecording) {
      ...
      setTimeout(() => {
        setRawInput('She had a good morning. Physical therapy went well. Pain levels are manageable. Visitors helped lift her spirits.');
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Share progress and keep everyone informed</p>
        </div>
        <Dialog>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Share information about today. AI will help compose a thoughtful update for the circle.
              </DialogDescription>
            </DialogHeader>
              ...
              <TabsContent value="text" className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Just jot down the key points. AI will compose a complete update.
                  </p>
                </div>
              </TabsContent>
            ...
            {generatedUpdate && (
              <div className="space-y-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" onClick={() => setGeneratedUpdate('')}>
                    Edit
                  </Button>
                </div>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500">
                      {update.createdAt.toLocaleDateString()} at {update.createdAt.toLocaleTimeString()}
                    </p>
                  </div>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {update.rawContent && (
                  <details className="text-sm">
                    ...
                    <div className="mt-2 p-3 bg-gray-50 rounded border border-gray-200">
                      <p className="text-gray-600 italic">{update.rawContent}</p>
                    </div>
                  </details>
                )}
                ...
                <div className="flex items-center gap-4 pt-2 border-t">
                  <Button variant="ghost" size="sm" className="gap-2">
                    ...
                    React {update.reactions.length > 0 && `(${update.reactions.length})`}
                  </Button>
                  <Button variant="ghost" size="sm" className="gap-2">
                    <MessageCircle className="w-4 h-4" />
                    Comment {update.comments.length > 0 && `(${update.comments.length})`}
                  </Button>
                </div>
                ...
                {update.comments.length > 0 && (
                  <div className="space-y-3 pt-2">
                    {update.comments.map((comment) => {
                      const commentAuthor = mockMembers.find((m) => m.id === comment.authorId);
                      return (
